...
public class P07BinarySearch {
    ...
    static int findPeakElement(int[] nums) {
        return firstTrue(0, nums.length - 1, m -> nums[m] > nums[m + 1]);
    }

    /** LC 69. 用 long 防 m*m 溢出。 */
    static int mySqrt(int x) {
        return firstTrue(0, x + 1, m -> (long) m * m > x) - 1;
    }
    ...
    public static void main(String[] args) {
        ...
        assert searchRotatedII(new int[] {1, 0, 1, 1, 1}, 0)
                && !searchRotatedII(new int[] {2, 5, 6, 0, 0, 1, 2}, 3);
        assert findMinRotated(new int[] {4, 5, 6, 7, 0, 1, 2}) == 0
                && findMinRotated(new int[] {11, 13}) == 11;
