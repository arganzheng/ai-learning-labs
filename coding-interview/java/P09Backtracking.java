...
public class P09Backtracking {
    ...
    private static void bt46(int[] nums, boolean[] used, Deque<Integer> path, List<List<Integer>> out) {
    ...
    private static void bt40(int[] c, int start, int remain, Deque<Integer> path, List<List<Integer>> out) {
        if (remain == 0) { out.add(new ArrayList<>(path)); return; }
        for (int i = start; i < c.length && c[i] <= remain; i++) {
    ...
    public static void main(String[] args) {
        ...
        assert permuteUnique(new int[]{1, 1, 2}).equals(List.of(List.of(1, 1, 2), List.of(1, 2, 1), List.of(2, 1, 1)));
        assert subsets(new int[]{1, 2, 3}).size() == 8;
        assert subsetsWithDup(new int[]{1, 2, 2}).size() == 6;
        assert combinationSum(new int[]{2, 3, 6, 7}, 7).equals(List.of(List.of(2, 2, 3), List.of(7)));
